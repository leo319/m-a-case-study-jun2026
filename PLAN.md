...
1. ✅ **Spine first.** Schemas + `deal_spec`/`source_registry`/`claims` + the deterministic citation verifier. Prove a fabricated URL gets caught before any agents exist. _Built in `/tool`; the deterministic citation verifier quarantines fabricated, dead, and quote-absent citations — proven by the spine smoke tests in `tests/test_spine.py`._
2. ✅ **Thin end-to-end slice on Cintas/UniFirst.** One area (target fundamentals), full loop, real data. _CC-native pipeline (skills in `.claude/skills/`) over the Python spine in `tool/`; Model C turn-by-turn gates + audit/steering log. **Live checkpoint done** (run `tool/runs/cintas_unifirst/20260607T163843Z/`): 21 verified / 0 quarantined off real SEC filings → narrative `research_brief.md` + grounded `preliminary_memo.md`. Live run surfaced + fixed: null-safe intake; SEC-compliant fetch UA + `inspect` read-path (WebFetch is 403-blocked by SEC); **Stage 2 Source Planning pulled forward** (built early); narrative brief/memo renderers (`render_brief`, `render-doc`) replacing bullet dumps._
3. ✅ **Widen research** to the full coverage checklist + verification report + gates. _Deal-agnostic 11-area `config/coverage_checklist.yaml`; claims carry an `area`; `coverage.py` emits `coverage_report.md` (searched/hits/gaps) — the §1 Comprehensiveness mechanism. Proven on mock (`tests/test_coverage.py`) and live: the Cintas/UF run now spans **9 of 11 areas, 100 verified claims, 0 required/recommended gaps** (only the two optional areas unsearched). Stage 2 Source Planning also built here, ahead of schedule._
   - _Post-M3 hardening (not a new milestone): **page-level decimal citations** + auto "sources consulted" appendix (`core/citations.py`, shared by the brief & memo renderers); dedicated **company-profile & industry-structure deep-dives** with tables; **Stage-2 deal-wide source planning + inline discovery sweep** (no separate scout agent). Re-grounding discipline (exact-unicode/contiguous quotes, page-level locators) baked into the research skill + worker. **Memo-depth pass** (PR #2): a **claim-utilization check** (`coverage.py` flags verified research inferences the memo left uncited, warned at the expert step) + sharper **must-answer §5/§6 rubrics** (valuation triangulation, ex-synergy multiple, accretion mechanism, precedent table incl. failed deals, people/execution risk, antitrust precedents)._
4. ✅ **Add experts + red-team.** _Stage 4 already produces the skeptical preliminary memo (single combined rationale+tail-risk expert with an embedded materiality filter — **not** split into two; deferred as unnecessary). **Stage 5 Red-Team & Finalize** built (PR #3): a fresh-context `skeptic` subagent attacks the memo's **judgments** (not its verified facts), mining the verified-but-unused claims for missed counter-evidence; the `red-team` skill resolves each finding (accept→edit/caveat, rebut→reasoning, logged to `redteam_critique.json`) and renders a separate fail-closed `final_memo.md`. **Scope calls:** one skeptic pass; **no research loop-back** (ungroundable gaps become caveats); **methodology method-cards deferred** (model already grounds the method; revisit when the M5 judge needs a shared rubric). Proven on cintas_unifirst: 14 findings → 9 accepted, 5 rebutted (incl. a benchmark rebutted because its evidence was quarantined — fail-closed holds against the red-team)._
   - _Post-M4 hardening (first-principles depth): the **method cards** (`tool/methodology/`) are
     now built — 1 meta card (first-principles trace, Cheng-style: reason before you frame) + 5
     bucket cards (industry / companies / deal+synergies / macro / why-this-deal), each a
     **facts→inferences bridge** grounded in an established framework (Porter, VRIO, Damodaran,
     lifecycle). They wire in WITHOUT a new stage: Stage 2 is repurposed as the **Plan** step,
     which derives deal-specific **`key_questions`** (what the static checklist misses for THIS
     deal) into `source_plan.json`; research carries them as worker mandate; the expert builds the
     "why" spine and answers them; the red-team hunts unanswered "whys", **closes what it can from
     the existing fact base** (grounded `rt_*` inference) and moves the rest to a **Limitations**
     section. **Scope calls hold:** still one skeptic pass and **no research loop-back** — the
     "ask why and drill" loop lives at red-team, bounded to the fact base._
